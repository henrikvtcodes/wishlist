...
export const RefTable = () => {
  ...
  const openDrawer = useRefDrawer((state) => state.openDrawer);

  return (
    <div className="px-4 sm:px-6 lg:px-8">
      <div className="sm:flex sm:items-center">
        <div className="flex items-center sm:flex-auto">
          <h1 className="text-xl font-semibold text-gray-900">Referrers</h1>
              ...
              <table className="min-w-full divide-y divide-gray-300">
                ...
                <tbody className="bg-white">
                  {refs?.map((ref, refIdx) => (
                    <tr
                      key={ref.id}
                      className={refIdx % 2 === 0 ? undefined : "bg-gray-50"}
                    >
                      ...
                      <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                        <button
                          onClick={() =>
                            openDrawer({ type: "edit", id: ref.id })
                          }
                          className="text-blue-600 hover:text-blue-900"
                        >
                          Edit<span className="sr-only">, {ref.name}</span>
                        </button>
                      </td>
                    </tr>
                  ))}
